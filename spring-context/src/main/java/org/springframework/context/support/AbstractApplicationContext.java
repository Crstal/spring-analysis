...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 1. 调用容器准备刷新的方法，获取容器当时的时间，同时给容器设置同步标志
			prepareRefresh();
			...
			try {
				...
				// 4、 为容器的某些子类指定特殊的 BeanFactoryPostProcessor 事件处理器
				// IoC容器允许BeanFactoryPostProcessor在容器实例化任何bean之前读取bean的定义(配置元数据)，并可以修改它
				...
				//6、 为 BeanFactory 注册 BeanPost 事件处理器. BeanPostProcessor 是 Bean 后置处理器， 用于监听容器触发的事件
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		...
		// 初始化 BeanFactory，并进行 XML 文件读取 ， 并将得到的 BeanFacotry 记录在当前实体的属性 中
		refreshBeanFactory();
	 ...
	 * 增加对 SpEL 语言的支持。（<property name="arg" value="#{表达式}">）
	 * 增加对属性编辑器的支持。
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// Allow for caching all ibean definition metadata, not expecting further changes.
